...
const Header = ({ hasNewData = false, notificationMessage = '', onClearNotifications = () => {}, adminUser = null, loginAdmin = () => {}, logoutAdmin = () => {}, pendingRegistrations = 0 }) => {
  ...
  const getUserDisplayName = (user) => {
    if (!user) return 'User'
    return user.firstName && user.lastName ? `${user.firstName} ${user.lastName}` : (user.firstName || 'Admin')
  }
  ...
  return (
    <header className="bg-linear-to-r from-slate-900 via-slate-800 to-slate-900 shadow-lg border-b border-slate-700">
      {/* Toast popup */}
      {showToast && (customMessage || notificationMessage) && (
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-6">
            ...
            <button type="button" onClick={() => onClearNotifications()} aria-label="Notifications" className="p-2.5 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white transition-all duration-200 relative group">
              ...
              {/* indicator: red for pending registrations, green for approved, blue for normal user */}
              <span className={`absolute top-1 right-1 h-2 w-2 rounded-full ${adminUser && pendingRegistrations > 0 ? 'bg-red-500 animate-pulse ring-2 ring-red-300' : adminUser ? 'bg-emerald-500' : 'bg-blue-500'} ${hasNewData && pendingRegistrations === 0 ? 'animate-pulse ring-2 ring-white/20' : ''}`}></span>
              <div className="absolute -bottom-10 -right-5 bg-slate-800 text-slate-200 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                {adminUser && pendingRegistrations > 0 ? `${pendingRegistrations} pending registration${pendingRegistrations > 1 ? 's' : ''}` : 
                 hasNewData ? (notificationMessage || (adminUser ? 'Admin activity' : 'Activity')) : 
                 (adminUser ? 'Signed in as admin' : 'Signed in as user')}
              </div>
            </button>
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="h-8 w-8 rounded-full overflow-hidden bg-slate-700 flex items-center justify-center">
                  {adminUser ? (
                    <div className="h-8 w-8 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center font-bold text-sm text-white">
                      {initials(getUserDisplayName(adminUser))}
                    </div>
                  ) : (
                    <div className="h-8 w-8 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center font-bold text-sm text-white">
                      SI
                ...
                <div className="hidden sm:block">
                  {adminUser ? (
                    <>
                      <p className="text-sm font-semibold">{getUserDisplayName(adminUser)}</p>
              ...
              {showDropdown && (
                <div role="menu" className="absolute right-0 mt-2 w-56 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-50">
                  <div className="px-4 py-3 border-b border-slate-700">
                    ...
                    <p className="text-xs text-slate-400">{adminUser ? getUserDisplayName(adminUser) : 'Not signed in'}</p>
                  </div>
                  <div className="py-2">
                    {adminUser ? (
                      <>
